...
public class CategoryController {
	...
	public String deleteSingleCategory(
			@RequestParam(value = "catId", required = true) Long categoryId, ModelMap model, HttpServletRequest request) {
	...
	public String getCategoryForEdit(
		...
		model.addAttribute("categoryForm", categoryService.getCategoryForEdit(categoryId));
		return "editcategory";
	}

	/**
	 * Handler Mapping/Method to handle Save Category Action by Admin for new categories and update existing categories
	 * @param category
	 * @param result
	 * @param errors
	 * @param model
	...
	public String onSubmitSaveCategory(
		...
		else if (userAction.equalsIgnoreCase("add") && categoryService.saveCategory(category, userAction))					
			model.addAttribute("saveCategoryMsg", category.getCategoryName()+Constants.categoryAddSuccess);
							
		else if (userAction.equalsIgnoreCase("edit") && categoryService.saveCategory(category, userAction))
		{			
			model.addAttribute("saveCategoryMsg",category.getCategoryName()+Constants.categoryEditSuccess);			
			return "editcategory";
		}
		else			
			result.rejectValue("categoryName", "categoryName.exist",Constants.categoryExists);			
